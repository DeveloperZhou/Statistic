...
//新建题目
function newQues() {
	window.self.location="./newMajor.jsp";
}

//删除题目
function delQues(sep) {
	if(confirm("您确实要删除该 班级 吗？删除后，不能再恢复。"))
	{
		window.self.location="./delMajor.jsp?"+"sep="+sep;
	}
}
						...
						<tr>
							...
							<td>
								<table class="table">
									...
									<tr>
										<td height="10">
											<span> <input type="button" name="sbmQues"  onclick="newQues();" value=" 新加班级 " /> </span>
										</td>
									</tr>
									<tr>
										<td colspan="5">
												...
												<table class="table2">
													<%
														...
														for (int i = 0; i < majorList.size(); i++) {
																...
																int sep = major.getMid();
																subCnt++;
															%>
													<tr>
														<td bgcolor="#CDE2FD" colspan=4>
															<span id="title<%=i%>"><%=i+1%>.<%=major.getMajorName()%></span>
															<br />
														</td>
													</tr>
													<td>
						<br/>
						<a onclick="delQues(<%=major.getMid()%>);" href="#">删除班级</a>
						<br />
					</td>
													<%
														}
