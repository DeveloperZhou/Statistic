...
public class ObjectBeanService {
	...
	public static int intsertObjectBean(ObjectBean bean) throws Exception {
		...
		try {
			...
			sql = "insert into wj_object(title,discribe,state,remark,anonymousFlag) values(?,?,?,?,?)";
			System.out.println(sql);
			stmt = con.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS);
			stmt.setString(1, bean.getTitle());
			stmt.setString(2, bean.getDiscribe());
			stmt.setInt(3, bean.getState());
			stmt.setString(4, bean.getRemark());
			stmt.setString(5, bean.getAnonymousFlag());
...
//	 查看问卷;
	public static List StuListObjectBean() {
		DBConnection dbcon = new DBConnection();
		Connection con = null;
		Statement stm = null;
		ResultSet rs = null;
		List objList=new LinkedList();
		try {
			String sql = "select oid,title,createtime,state from wj_object where state = 1 order by oid desc";
			con=dbcon.getConnection();
			stm=con.createStatement();
			rs = stm.executeQuery(sql);
			while(rs.next()){
