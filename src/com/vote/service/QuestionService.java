...
public class QuestionService {
	...
	public List<Question> litQuesByOid(int oid) {
		...
		String sql = "select seq,content,qtype,point from wj_question where oid='"
	...
	public int addQues(int oid, String content, int qtype, int seq,int point) {
		DBConnection dbcon = null;
		Connection con = null;
		Statement stm = null;
		ResultSet rs = null;
		String sql = "insert into wj_question(oid,content,qtype,seq,point) values('"
				+ oid + "','" + content + "','" + qtype + "','"
				+ seq + "','" + point + "')";
		try {
			dbcon=new DBConnection();
...
//	 编辑题目的时候 先的到它的题目TITLE ,选项类型,选项内容
	public Question getQuesBySeq(int seq, int oid) {
		...
		ResultSet rs = null;
		Question ques=new Question();
		String sql = "select content,qtype,point from wj_question where oid = '" + oid
				+ "' and seq = '" + seq + "'";
		try {
			dbcon=new DBConnection();
			con=dbcon.getConnection();
